...
const publicRoutes = new Map<string, RegExp[]>([
  ...
  ["/api/phase8/contact", [/enforcePublicRateLimit|recordProductEvent|contact/i, /schema\.parse|z\.object/]],
  ["/api/phase8/pilot", [/enforcePublicRateLimit/, /schema\.parse/]],
]);
...
const validationPatterns = [/z\.object/, /schema\.parse/, /readJson\(/, /request\.json\(/, /request\.text\(\)/, /request\.formData\(\)/];
...
test("public write routes apply public rate limiting", () => {
  const publicWriteRoutes = [
    ...
    "app/api/phase8/contact/route.ts",
    "app/api/phase8/pilot/route.ts",
  ];
