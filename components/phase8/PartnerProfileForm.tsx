...
export function PartnerProfileForm({ defaultWebsite = "", defaultMarkets = [] }: { defaultWebsite?: string; defaultMarkets?: string[] }) {
  ...
  async function submit(formData: FormData) {
    const response = await fetch("/api/phase8/partner", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ website: formData.get("website"), focusMarkets: String(formData.get("markets") ?? "").split(",").map((value) => value.trim()).filter(Boolean) }) });
