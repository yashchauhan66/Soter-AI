  ...
  return <div className="space-y-7">
    <section className="rounded-xl border border-slate-800 bg-slate-950/60 p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-slate-200">Choose exactly where Soter activates. Browser monitoring is limited to enabled URL patterns; desktop IDEs and direct APIs use official plugins or the transparent local proxy.</p>
...
function DestinationCard({ destination, onPatch, onDelete }: { destination: AIDestinationPolicy; onPatch: (patch: Partial<AIDestinationPolicy>) => void; onDelete: () => void }) {
  ...
  return <article className="rounded-xl border border-slate-800 bg-slate-950/60 p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="font-semibold text-slate-100">{local.name}</h2><p className="mt-1 break-all text-xs text-slate-300">{patterns}</p></div><label className="flex items-center gap-2 text-xs text-slate-300"><input type="checkbox" checked={local.enabled} onChange={(e) => { change("enabled", e.target.checked); onPatch({ enabled: e.target.checked }); }} /> Enabled</label></div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="grid gap-2 text-sm text-slate-300"><span className="text-xs font-medium uppercase tracking-wide text-slate-300">{label}</span>{children}</label>; }
...
function Boundary({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) { return <div className="rounded-xl border border-slate-800 p-5"><span className="text-cyan-300">{icon}</span><h3 className="mt-3 font-semibold">{title}</h3><p className="mt-2 text-sm text-slate-200">{children}</p></div>; }
