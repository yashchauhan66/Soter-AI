...
import { ensureCustomerOnboarding } from "@/lib/ops/onboarding";
...
export async function OnboardingExperience({ type, title, description }: { type: "BETA" | "AGENCY" | "ENTERPRISE"; title: string; description: string }) {
  ...
  return <div><p className="eyebrow">Launch onboarding</p><h1 className="mt-2 text-3xl font-bold">{title}</h1><p className="mb-8 mt-3 max-w-3xl text-slate-200">{description}</p><OnboardingWizard onboardingId={onboarding.id} type={type} projectId={project.id} initialSteps={onboarding.stepEvents} /></div>;
}
