...
import { ONBOARDING_STEPS, ONBOARDING_STEP_CONTENT, type OnboardingStepKey } from "@/lib/ops/onboarding";
  ...
  return <div>
    ...
    <div className="divide-y divide-slate-800 border-y border-slate-800">
      {ONBOARDING_STEPS.map((stepKey, index) => { const content = ONBOARDING_STEP_CONTENT[stepKey]; const state = states.get(stepKey); return <div className="grid gap-3 py-4 sm:grid-cols-[32px_1fr_auto] sm:items-center" key={stepKey}>
        <span className={`flex h-8 w-8 items-center justify-center rounded-full border ${state ? "border-cyan/40 bg-cyan/10 text-cyan" : "border-slate-700 text-slate-300"}`}>{state === "COMPLETED" ? <Check size={16} /> : state === "SKIPPED" ? <SkipForward size={15} /> : <Circle size={12} />}</span>
