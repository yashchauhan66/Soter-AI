...
export function PartnerProfileForm({ defaultWebsite = "", defaultMarkets = [] }: { defaultWebsite?: string; defaultMarkets?: string[] }) {
  ...
  return <form action={submit} className="grid gap-4"><label className="text-sm text-slate-300">Website<input name="website" type="url" defaultValue={defaultWebsite} maxLength={2048} className="input mt-2" placeholder="https://agency.example" /></label><label className="text-sm text-slate-300">Focus markets<input name="markets" defaultValue={defaultMarkets.join(", ")} maxLength={500} className="input mt-2" placeholder="SaaS, clinics, coaching" /></label><button className="button-primary">Save partner profile</button>{message && <p className="text-sm text-cyan">{message}</p>}</form>;
}
