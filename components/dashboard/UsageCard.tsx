    ...
    <div className="card p-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm text-slate-300">Plan usage</p>
        </div>
        <span className="rounded-full bg-slate-800 px-3 py-1 text-xs font-semibold">{plan}</span>
      </div>
      <p className="mt-3 text-2xl font-bold">{used.toLocaleString("en-IN")}<span className="text-sm font-normal text-slate-300"> / {limit.toLocaleString("en-IN")}</span></p>
      ...
      {exceeded
        ...
        : warning
          ...
          : <p className="mt-3 text-xs text-slate-300">Healthy usage, well under quota.</p>}
    </div>
  );
}
