    ...
    <div className="space-y-5">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(360px,0.95fr)]">
          <section className="card overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 px-5 py-4">
              <div><h2 className="font-semibold">Code or unified diff</h2><p className="mt-1 text-xs text-slate-300">Max 24 KB · raw code is not stored</p></div>
            ...
            <div className="grid gap-3 border-t border-slate-800 p-5 sm:grid-cols-2">
              <label className="text-xs text-slate-200">Environment<select className="input mt-1 py-2 text-sm" value={environment} onChange={(event) => setEnvironment(event.target.value as typeof environment)}><option value="development">Development</option><option value="staging">Staging</option><option value="production">Production</option></select></label>
      ...
      {tab === "github" && (
        <section className="card overflow-hidden">
          ...
          <div className="border-t border-slate-800 p-5 text-sm text-slate-200">The check reviews added/modified lines, annotates findings in the job log, and fails when critical findings or multiple high-severity findings are detected.</div>
        </section>
      )}
      ...
      {tab === "compliance" && (
        <section className="card p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><h2 className="text-lg font-semibold">Secure development evidence report</h2><p className="mt-2 text-sm text-slate-200">A portable Markdown report mapping review evidence to OWASP, SOC 2, ISO 27001, and NIST SSDF controls.</p></div>
          ...
          {!result ? <div className="mt-5 rounded-xl border border-dashed border-slate-700 p-8 text-center text-sm text-slate-300">Run a security review first to generate control evidence.</div> : (
            <div className="mt-5 grid gap-4 lg:grid-cols-2">
              {result.compliance.map((item) => <article className="rounded-xl border border-slate-800 bg-slate-950/40 p-4" key={item.framework}><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">{item.framework}</h3><span className={item.status === "PASS" ? "text-emerald-300" : "text-amber-300"}>{item.status}</span></div><p className="mt-2 text-sm text-slate-200">{item.note}</p><p className="mt-3 text-xs text-slate-300">{item.controls.join(" · ")}</p></article>)}
              <p className="text-xs text-slate-300 lg:col-span-2">{result.disclaimer}</p>
            </div>
          )}
        </section>
      )}
...
function ReviewResults({ result }: { result: CodeReviewResult | null }) {
  if (!result) return <section className="card flex min-h-[540px] items-center justify-center p-8 text-center"><div><Sparkles className="mx-auto text-cyan" size={28} /><h2 className="mt-4 font-semibold">Security findings will appear here</h2><p className="mt-2 max-w-sm text-sm text-slate-300">The review combines source patterns with deployment, data, auth, and AI-generation context.</p></div></section>;
  ...
  return <section className="space-y-4">
    ...
    <div className="grid grid-cols-4 gap-2">{(["CRITICAL", "HIGH", "MEDIUM", "LOW"] as const).map((severity) => <div className="card p-3 text-center" key={severity}><p className="text-xl font-bold">{result.counts[severity]}</p><p className="mt-1 text-[10px] text-slate-300">{severity}</p></div>)}</div>
    <div className="space-y-3">{result.findings.map((finding) => <article className="card p-4" key={finding.id}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-cyan">{finding.ruleId} · {finding.category}</p><h3 className="mt-1 font-semibold">{finding.title}</h3></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${severityClass(finding.severity)}`}>{finding.severity}</span></div><p className="mt-2 text-sm text-slate-200">{finding.description}</p><pre className="mt-3 overflow-auto rounded-lg bg-slate-950/70 p-3 text-xs text-slate-300">Line {finding.line}: {finding.evidence}</pre><div className="mt-3 rounded-lg border border-cyan/10 bg-cyan/5 p-3 text-sm text-slate-300"><strong className="text-cyan">Fix:</strong> {finding.remediation}</div><p className="mt-3 text-[11px] text-slate-300">{finding.standards.join(" · ")}</p></article>)}{result.findings.length === 0 && <div className="card p-6 text-center text-sm text-emerald-300">No high-confidence findings detected. Continue with dependency scanning, tests, and human review.</div>}</div>
  </section>;
}
