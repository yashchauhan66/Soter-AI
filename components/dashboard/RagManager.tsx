...
export function RagManager({ projectId, collections }: { projectId: string; collections: Array<{ id: string; name: string }> }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-2">
    <form action={createCollection} className="card p-6"><h2 className="font-semibold">New collection</h2><input name="name" required minLength={2} maxLength={200} className="input mt-4" placeholder="Support knowledge base" /><textarea name="description" className="input mt-3" maxLength={500} placeholder="Purpose and approved sources" /><button className="button-primary mt-3">Create collection</button></form>
    <form action={upload} className="card p-6"><h2 className="font-semibold">Scan document</h2><select name="collectionId" required className="input mt-4"><option value="">Select collection</option>{collections.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input name="file" type="file" accept=".txt,.md,.pdf,text/plain,text/markdown,application/pdf" required className="input mt-3" /><p className="mt-2 text-xs text-slate-300">TXT, Markdown, and text-extractable PDF. Raw secrets are never stored.</p><button className="button-primary mt-3">Upload and scan</button></form>
