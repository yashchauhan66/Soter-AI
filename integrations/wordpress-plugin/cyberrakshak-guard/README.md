# CyberRakshak Guard — WordPress Plugin

OWASP LLM Top 10 aligned AI security gateway for WordPress chatbots. Guards
chatbot **input** and **output** server-side through the CyberRakshak Guard API.
...
1. Copy the `cyberrakshak-guard` folder into `wp-content/plugins/`.
2. Activate **CyberRakshak Guard** in the Plugins screen.
3. Open **Settings → CyberRakshak Guard**.
...
```
Visitor message
   → WordPress REST route (/wp-json/cyberrakshak/v1/guard-input)   [server-side]
      → CyberRakshak Input Guard (x-api-key sent here, never to the browser)
   → if allowed, your chatbot generates a reply
   → WordPress REST route (/wp-json/cyberrakshak/v1/guard-output)  [server-side]
      → CyberRakshak Output Guard
...
```php
$result = cyberrakshak_guard_input( $user_message );
if ( $result['blocked'] ) {
...
| Route | Method | Body |
| --- | --- | --- |
| `/wp-json/cyberrakshak/v1/guard-input` | POST | `{ "text": "…" }` |
| `/wp-json/cyberrakshak/v1/guard-output` | POST | `{ "text": "…" }` |
