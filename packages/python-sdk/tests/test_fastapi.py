...
from soter import Soter  # noqa: E402
from soter.fastapi import create_chat_route  # noqa: E402
from conftest import FakeSession  # noqa: E402


def _make_app():
    session = FakeSession()
    guard = Soter(api_key="ck_test_key_123456", base_url="http://localhost:3000", session=session)
    app = FastAPI()
    app.add_api_route(
        "/chat",
