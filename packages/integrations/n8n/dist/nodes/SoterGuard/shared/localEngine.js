...
exports.LOCAL_RULE_COUNT = exports.US_SSN_TOKEN = exports.LOCAL_ENGINE_LIMITATIONS = exports.LOCAL_ENGINE_VERSION = void 0;
exports.foldText = foldText;
exports.redactUsSsn = redactUsSsn;
exports.redactLocal = redactLocal;
exports.analyzeLocal = analyzeLocal;
exports.scoreRagDocumentLocal = scoreRagDocumentLocal;
exports.compareEgressLocal = compareEgressLocal;
exports.checkToolCallLocal = checkToolCallLocal;
exports.LOCAL_ENGINE_VERSION = "1.0.0";
/**
 ...
 */
exports.LOCAL_ENGINE_LIMITATIONS = [
...
const INVISIBLE_CHARS = /[\u00ad\u034f\u061c\u180b-\u180e\u200b-\u200f\u202a-\u202e\u2060-\u2064\u206a-\u206f\ufeff]|[\u{E0000}-\u{E007F}]/gu;
 ...
 */
function foldSpacedLetters(text) {
 ...
 */
function detectionVariants(text) {
...
const INSTRUCTION_TARGET = "(?:all\\s+|any\\s+|the\\s+|your\\s+|every\\s+)*(?:previous|prior|preceding|above|earlier|initial|original|system|existing|current)\\s*(?:instruction|instructions|prompt|prompts|rule|rules|direction|directions|command|commands|guideline|guidelines|context|constraint|constraints|restriction|restrictions|policy|policies)";
...
const DISCLOSE_VERB = "(?:show|reveal|print|repeat|output|display|dump|echo|list|enumerate|recite|state|spell\\s+out|write\\s+out|read\\s+back|translate|tell\\s+me|give\\s+me|what\\s+(?:are|is|was|were))";
...
const UNSAFE_PERSONA = "(?:unrestricted|unfiltered|uncensored|unbounded|unlimited|amoral|immoral|jailbroken|jail\\s*broken|evil|rogue|malicious|lawless|criminal|villainous|without\\s+(?:any\\s+)?(?:morals?|ethics|limits|rules|filters|restrictions))";
const LOCAL_RULES = [
    // -- Prompt injection -----------------------------------------------------
    {
        ...
        patterns: [
            ...
            new RegExp(`${INSTRUCTION_VERB}\\b[^.?!]{0,30}\\byour\\s+(?:[\\w-]+\\s+){0,2}` +
                `(?:instruction|instructions|rule|rules|prompt|prompts|directive|directives|guideline|guidelines|constraint|constraints|restriction|restrictions|programming)\\b`, "i"),
            ...
            new RegExp(`${INSTRUCTION_VERB}\\b[^.?!]{0,40}\\b(?:instruction|instructions|rule|rules|thing|things|direction|directions|command|commands|order|orders|guideline|guidelines)\\s+(?:that\\s+)?you\\s+(?:were|have\\s+been|had\\s+been|was)\\s+(?:given|told|taught|programmed|trained|instructed|provided|handed)\\b`, "i"),
    ...
    },
    {
        ...
        patterns: [
            ...
            new RegExp(`\\b(?:act|behave|respond|reply|pretend|simulate|imagine|emulate|impersonate|role[\\s-]?play|play)\\s+` +
                `(?:as\\s+|to\\s+be\\s+|that\\s+)?(?:if\\s+)?(?:you\\s+(?:are|were|re)\\s+)?(?:an?\\s+|the\\s+)?` +
                `(?:\\w+\\s+){0,2}${UNSAFE_PERSONA}\\b`, "i"),
    ...
    },
    {
        ...
        direction: "INPUT",
        patterns: [
            new RegExp(`${DISCLOSE_VERB}\\b[^.?!]{0,50}\\b(?:your\\s+)?(?:system\\s+prompt|initial\\s+instructions?|original\\s+instructions?|` +
                `system\\s+message|hidden\\s+(?:prompt|instructions?)|prompt\\s+template|developer\\s+message)`, "i"),
...
exports.US_SSN_TOKEN = "[REDACTED_US_SSN]";
...
const US_SSN_LABELLED = /((?:SSNs?|social[\s-]?security(?:[\s-]?(?:number|no\.?|#))?)\s*[:=#-]?\s*)((?!000|666|9\d\d)\d{3}[-\s]?(?!00)\d{2}[-\s]?(?!0000)\d{4})\b/gi;
/**
 ...
 */
function redactUsSsn(text) {
    let count = 0;
    let output = text.replace(US_SSN_LABELLED, (_match, label) => {
        count += 1;
        return `${label}${exports.US_SSN_TOKEN}`;
    });
    output = output.replace(US_SSN_DASHED, () => {
        count += 1;
        return exports.US_SSN_TOKEN;
    });
...
const REDACTION_RULES = [
    ...
    },
    {
        ...
        severity: "CRITICAL",
        pattern: /\b(?:sk-(?:proj-|ant-)?[A-Za-z0-9_-]{16,}|sk_(?:live|test)_[A-Za-z0-9]{10,}|rk_(?:live|test)_[A-Za-z0-9]{10,}|gh[pousr]_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|npm_[A-Za-z0-9]{30,}|xox[baprs]-[A-Za-z0-9-]{10,}|AIza[A-Za-z0-9_-]{30,}|ya29\.[A-Za-z0-9._-]{20,}|dop_v1_[a-f0-9]{60,}|shpat_[a-f0-9]{30,}|glpat-[A-Za-z0-9_-]{20,})\b/g,
    ...
    },
    {
        ...
        pattern: /\b(?:api[_-]?key|apikey|access[_-]?token|auth[_-]?token|bearer[_-]?token|client[_-]?secret|secret[_-]?key|private[_-]?token|password|passwd|pwd)\b\s*[:=]\s*["']?([A-Za-z0-9_\-./+=]{12,})["']?/gi,
    ...
    },
    {
        ...
        pattern: /\b[\w.-]{3,64}@(?:oksbi|okhdfcbank|okicici|okaxis|paytm|ybl|ibl|axl|upi|apl|jio|airtel|freecharge|hdfcbank|icici|axisbank|sbi)\b/gi,
        token: "[REDACTED_UPI]",
    },
    {
        type: "PII_DETECTED",
    ...
    },
    {
        ...
        label: "Phone number",
        severity: "MEDIUM",
        pattern: /(?:\+\d{1,3}[\s-]?)?(?:\(\d{3}\)|\d{3})[\s.-]\d{3}[\s.-]\d{4}\b|\b(?:\+91[\s-]?|0)?[6-9]\d{4}[\s-]?\d{5}\b|\+\d{10,15}\b/g,
...
function redactLocal(text) {
    ...
    for (const rule of REDACTION_RULES) {
        let hits = 0;
        output = output.replace(rule.pattern, (match, ...groups) => {
            ...
            const captured = typeof groups[0] === "string" ? groups[0] : undefined;
 ...
 */
function analyzeLocal(text, direction = "INPUT") {
    ...
    for (const rule of LOCAL_RULES) {
        ...
        if (!hit)
            continue;
        if (rule.not?.some((pattern) => variants.some((variant) => pattern.test(variant))))
            continue;
    ...
    const findings = [...byType.entries()].map(([type, entry]) => ({
        type,
    ...
    const worstSeverity = findings.reduce((worst, finding) => (worst === null || SEVERITY_RANK[finding.severity] > SEVERITY_RANK[worst] ? finding.severity : worst), null);
    ...
    const blocking = attackFindings.filter((finding) => finding.severity === "HIGH" || finding.severity === "CRITICAL");
    let action;
    if (blocking.length > 0)
        action = "BLOCK";
    else if (attackFindings.length > 0)
        action = "REVIEW";
    else if (privacyFindings.length > 0)
    ...
    const primary = blocking[0] ?? attackFindings[0] ?? privacyFindings[0] ?? findings[0] ?? null;
    return {
        allowed,
        action,
        riskScore,
        ...
        categoryConfidence: Object.fromEntries(findings.map((finding) => [finding.type, Number((SEVERITY_RANK[finding.severity] / 4).toFixed(2))])),
        latencyMs: Date.now() - startedAt,
        engine: "local",
        engineVersion: exports.LOCAL_ENGINE_VERSION,
        engineLimitations: exports.LOCAL_ENGINE_LIMITATIONS,
    };
}
function buildLocalReason(action, findings) {
    if (findings.length === 0) {
        return "No risk detected by the local pattern engine.";
    }
    const worst = findings.reduce((current, finding) => SEVERITY_RANK[finding.severity] > SEVERITY_RANK[current.severity] ? finding : current);
 ...
 */
function scoreRagDocumentLocal(text, documentId, source) {
    const analysis = analyzeLocal(text, "INPUT");
    const untrustedSource = ["email", "url", "unknown"].includes(typeof source === "string" ? source.toLowerCase() : "unknown");
    let score = Math.max(0, 100 - analysis.riskScore);
    if (untrustedSource)
        score = Math.max(0, score - 10);
    const documentBorne = analysis.findings.filter((finding) => ATTACK_TYPES.has(finding.type) && (finding.severity === "HIGH" || finding.severity === "CRITICAL"));
    if (documentBorne.length > 0)
    ...
    const recommendedAction = trustLevel === "QUARANTINED" ? "QUARANTINE" : hasPrivacy ? "REDACT_AND_INDEX" : trustLevel === "TRUSTED" ? "INDEX" : "REVIEW";
    return {
        documentId,
        trustScore: score,
        trustLevel,
...
exports.LOCAL_RULE_COUNT = LOCAL_RULES.length + REDACTION_RULES.length + 1;
// ---------------------------------------------------------------------------
// Egress comparison
//
 ...
 */
function compareEgressLocal(content, sources) {
    ...
    for (const source of sources) {
        ...
        const sourceText = source.content.length > EGRESS_COMPARE_MAX_CHARS
    ...
    return {
        ...
        engine: "local",
        engineNote: "Local comparison uses the source text supplied inline. Sources given as an id only cannot be " +
 ...
 */
function sharedVerbatimRun(a, b, minLength) {
    if (minLength <= 0)
        return 0;
    ...
    for (let i = 1; i < minLength; i++)
        power = Math.imul(power, EGRESS_HASH_BASE);
    let hash = 0;
    for (let i = 0; i < minLength; i++)
        hash = (Math.imul(hash, EGRESS_HASH_BASE) + needle.charCodeAt(i)) | 0;
    ...
    let scan = 0;
    for (let i = 0; i < minLength; i++)
        scan = (Math.imul(scan, EGRESS_HASH_BASE) + haystack.charCodeAt(i)) | 0;
    ...
    for (let start = 0;; start++) {
        for (let candidate = head.get(scan) ?? -1; candidate !== -1; candidate = next[candidate]) {
            ...
            if (!equal)
                continue; // hash collision, not a shared run
            let from = 0;
            while (candidate - from - 1 >= 0 &&
                start - from - 1 >= 0 &&
                needle.charCodeAt(candidate - from - 1) === haystack.charCodeAt(start - from - 1)) {
                from += 1;
            }
            let to = 0;
            while (candidate + minLength + to < needle.length &&
 ...
 */
function checkToolCallLocal(input) {
    ...
    const spoken = (value) => (value ?? "").replace(/[_\-.]+/g, " ").trim();
    ...
    return {
        ...
        findings,
        engine: "local",
        engineNote: "Local mode checks the tool payload and the reach of the call. It does not verify agent identity — " +
