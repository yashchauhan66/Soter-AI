...
export async function endSecuritySpan(span: TraceSpan, status: "OK" | "ERROR", extra: TraceSpan["attributes"] = {}) {
  ...
  if (endpoint) {
    await fetch(`${endpoint.replace(/\/$/, "")}/v1/traces`, { method: "POST", headers: { "content-type": "application/json", ...(process.env.OTEL_EXPORTER_OTLP_HEADERS ? Object.fromEntries(process.env.OTEL_EXPORTER_OTLP_HEADERS.split(",").map((pair) => pair.split("=", 2) as [string, string])) : {}) }, body: JSON.stringify({ resourceSpans: [{ resource: { attributes: [{ key: "service.name", value: { stringValue: "soterai" } }] }, scopeSpans: [{ spans: [record] }] }] }), signal: AbortSignal.timeout(5_000) }).catch((error) => console.warn("[SoterAI] OpenTelemetry export failed", error));
  }
  return record;
}
