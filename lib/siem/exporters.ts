...
class SignedWebhookExporter implements SiemExporter {
  constructor(private readonly endpoint: string, private readonly secretHash?: string) {}
  async export(event: ExportableSecurityEvent) {
    const endpoint = await assertPublicOutboundUrl(this.endpoint);
    const body = JSON.stringify(redactedWebhookPayload(event));
    const signed = this.secretHash ? signWebhookPayload(body, this.secretHash) : null;
...
export function createSiemExporter(provider: string, endpoint: string, token?: string): SiemExporter {
  if (provider === "webhook") return new SignedWebhookExporter(endpoint, token);
...
export async function processSiemDelivery(deliveryId: string) {
  ...
  if (delivery.integration.provider === "webhook") {
    token = decodeWebhookConfig(delivery.integration.encryptedToken).secretHash;
  } else if (delivery.integration.encryptedToken && delivery.integration.tokenKeyVersion) {
