...
export async function persistGuardResult(input: {
  ...
  if (owner?.id) {
    ...
    if (result.action === "BLOCK" && result.riskScore >= 80 && owner.email) {
      ...
      void sendTemplateEmail({ to: owner.email, template: "high-risk-alert", data: { projectName: project?.name ?? input.projectId } }).catch((error) => console.error("[SoterAI] High-risk alert email failed for project", input.projectId, error));
    }
  }
  ...
  if (project?.organizationId && result.action !== "ALLOW") {
    ...
    void emitSecurityEvent({ organizationId: project.organizationId, projectId: input.projectId, eventType, severity, riskTypes: result.riskTypes, action: result.action, source: `guard.${input.direction.toLowerCase()}`, metadata: { riskScore: result.riskScore, direction: input.direction } }).catch((error) => console.error("[SoterAI] Security event emission failed for project", input.projectId, error));
  }
  ...
  if (project?.organizationId) {
    void markGuardActivation({
      ...
      userId: owner?.id,
      action: result.action,
    }).catch((error) => console.error("[SoterAI] Activation tracking failed for project", input.projectId, error));
  }
  ...
  }).catch((error) => console.error("[SoterAI] Webhook dispatch failed for project", input.projectId, error));
  ...
  if (project?.organizationId && project.plan) {
    void maybeFireUsageWebhook(input.projectId, project.organizationId, project.plan, project.organization?.quotaOverride, owner?.email ?? undefined, project.name).catch((error) =>
      console.error("[SoterAI] Usage webhook failed for project", input.projectId, error),
    );
  } else {
