...
const inputRules: PatternRule[] = [
  ...
  { pattern: /(?:internal debug request|debug request|maintenance debug).{0,140}(?:print|show|reveal|return).{0,120}(?:exact )?(?:rules|instructions?|policies).{0,140}(?:before|conversation started|this conversation|startup)/i, label: "Debug pretext prompt disclosure", message: "Uses a debug pretext to request hidden startup instructions.", severity: "HIGH", score: 45 },
  { pattern: /summari[sz]e.{0,100}(?:confidential|hidden|internal|private).{0,100}(?:setup|system|developer|tool|policy).{0,100}(?:instructions?|prompts?|policies|rules)/i, label: "Confidential setup summary request", message: "Requests a summary of hidden setup or policy instructions.", severity: "HIGH", score: 45 },
  { pattern: /what exact rules.{0,140}(?:cannot see|can't see|not visible|hidden from me|invisible to me)/i, label: "Hidden rule paraphrase", message: "Asks for exact hidden rules using indirect wording.", severity: "HIGH", score: 45 },
  { pattern: /print everything above.{0,160}(?:user message starts here|line|delimiter|marker)/i, label: "Pre-user context extraction", message: "Requests text from hidden context before the user message.", severity: "HIGH", score: 45 },
  { pattern: /(?:fictional story|story where|game).{0,180}(?:assistant|ai).{0,160}(?:leaks?|reveals?|prints?).{0,120}(?:secret instructions?|hidden instructions?|full leaked text|system prompt)/i, label: "Narrative prompt leakage", message: "Uses fiction or game framing to extract hidden instructions.", severity: "HIGH", score: 45 },
