...
// Hinglish (romanized Hindi) adversarial cues. Requires BOTH an adversarial
// verb (bypass/override/extract) AND a security/system target. This keeps
// benign commerce Hinglish ("order status batao", "invoice bhejo") from
// firing, since commerce targets are excluded from HINGLISH_TARGET.
const HINGLISH_INTENT =
  /\b(?:bhool\s*(?:jao|ja|jaa)|bhul\s*(?:jao|ja|jaa)|bhula\s*do|hata\s*do|hatao|hata\s*doo|band\s*kar(?:o|do|doo)?|disable\s*kar(?:o|do|doo)?|off\s*kar(?:o|do|doo)?|tod\s*do|todo|todo\s*do|ignore\s*kar(?:o|do|doo)?|override\s*kar(?:o|do)?|bypass\s*kar(?:o|do)?|unlock\s*kar(?:o|do)?|nikaal(?:o|ke|kar|lo)?|nikaal\s*(?:ke|kar|lo)|dikha(?:o|do|doo)?|dikha\s*(?:do|doo)|dump\s*kar(?:o|do)?|likh\s*kar\s*do|likh\s*(?:do|kar)|paste\s*kar(?:o|do)?|forward\s*kar\s*do|forward\s*kar(?:o|do)?|bhej\s*(?:do|doo)|bhej\s*(?:de|to)|print\s*kar(?:o|do)?|chhaap(?:o|do)?|reveal\s*kar(?:o|do)?|expose\s*kar(?:o|do)?|copy-?paste\s*kar(?:o|do)?|export\s*kar(?:o|do)?|temp(?:orary)?\s*tor\s*par\s*(?:nikaal|hata)|temporary\s*tor\s*par\s*(?:nikaal|hata)|rules?\s*(?:todo|tora|toro|tod|phodo|phod|pheko|phek|maaro|uDao|uDaao)|filter\s*(?:hatao|hata|hatado|nikalo|band|todo|tora|toro)|bina\s+(?:kisi\s+|koi\s+|ek\s+|sabhi\s+|saare\s+)?(?:censor|rok|restriction|filter|parhach|nakaab|pardi|rok)\s+ke|protocol\s+(?:suspend|band|hatao|hata|tod|phodo)|seedha\s+(?:answer|jawaab|output|reply|response)\s+(?:chahiye|do|bhejo|dikhao|batao)|bina\s*roke\s*(?:bolo|do|bhejo|batao|dikha)|sab\s*(?:kuch\s*)?(?:reveal|expose|dikha|bata|nikaal)\s*(?:do|kar)|poora\s*(?:system|database|config|prompt)\s*(?:nikaal|dikha|print|dump|bhej)|verbatim\s*(?:likh|chhaap|print|dikha|bhej)|saara?\s*(?:hidden|secret|internal|confidential)\s*(?:data|config|memory|context)\s*(?:nikaal|dikha|bhej|dump)|word\s*by\s*word\s*likh|mujhe\s*do|dono\s*do|delete\s*kar(?:o|do)?|suspend\s*kar(?:o|do)?|mode\s*off|restrict\s*nahi\s*kiya|god\s*mode|admin\s*override|jailbreak\s*(?:mode|kar)|pretend\s*(?:karo|kar)|roleplay\s*(?:karo|kar)\s*(?:unlocked|unrestricted|no\s*limit))\b/i;
const HINGLISH_TARGET =
  /\b(?:rules?|niyam|safety|suraksha|moderation|restrictions?|filters?|system\s*prompt|prompt|instructions?|guidelines?|api\s*keys?|credentials?|tokens?|database|memory|config(?:uration)?|environment(?:\s*variables?)?|env(?:ironment)?|secret|confidential|admin|backend|access\s*tokens?|private\s*keys?|internal\s*(?:config|context|instructions?|notes?|logs?|records?)|user\s*(?:data|records?|list)|chat\s*history|internal\s*context|unlocked|censor(?:ed)?|compliance(?:\s*mode)?|ethical\s*constraints?|guardrails?|developer\s*(?:mode|message|instructions?)|hidden\s*(?:prompt|instructions?|context|config|system\s*rules?)|initial\s*(?:prompt|instructions?|message)|base\s*prompt|operating\s*(?:instructions?|parameters?)|system\s*(?:message|rules?)|training\s*(?:data|guidelines?)|alignment|content\s*(?:policy|policies)|policy\s*me|creators?\s*(?:ki|ke)\s*(?:secret|notes?)|protocols?|safety\s*notes?|role\s*description|private\s*prompt|highest\s*priority|khul\s*kar\s*bolo|bandhan|chhoot|bina\s*roke|bina\s*bandhan)\b/i;
...
const rules: IntentRule[] = [
  ...
  // ── Jailbreak: educational/research pretext ─────────────────────────────
  {
    ...
    score: 45,
    severity: "HIGH",
    // Suppress reference-QA framing: quoting a passage/article as a comprehension
    // question is not a pretext for operational detail.
    not: [
      /\b(?:given|based\s+on|according\s+to|using|with|from|in)\s+(?:this|the|a|an|the\s+following|the\s+below|the\s+provided|the\s+given)\s+(?:text|article|passage|paragraph|reference|document|summary|section|excerpt|post|review|memo|sentence)\b|\b(?:from|using|given)\s+(?:this|the)\s+passage\b|\bextract\s+(?:from\s+)?(?:this|the|the\s+below|the\s+following)\s+(?:article|text|passage|paragraph|document)\b/i,
    ],
    all: [
      ...
      // Requires an explicit dangerous-content noun (or an academic-purpose tail),
      // so generic adjectives alone ("complete", "exact", "full") no longer fire.
      /\b(?:method\s+for\s+the\s+attack\b|phishing\s+kit(?:\s+code)?\b|(?:give|provide|supply|need|receive|output)\s+(?:me\s+)?(?:the\s+)?(?:actual|real|complete|full|working)?\s*malware\s+(?:source\s+)?code\b|ransomware\s+(?:routine|code|source|strain|payload)\b|functioning\s+ransomware\b|exploit(?:\s+steps?)?\b|(?:working|weaponized|harmful|malicious|attack|exploit|exact|complete|full)\s+payload\b|weaponized\b|synthesis\s+instructions?\b|(?:exact|precise|specific|full|complete|recommended)\s+dosages?\b|preparation\s+steps?\b|performs?\s+the\s+intrusion\b|attack\s+(?:chain|recipe|steps?)\b|offensive\s+(?:technique|method|detail|capability|tool)\b|operational\s+(?:detail|tradecraft|content)\b|tested\s+code\b|code\s+that\s+performs\b|(?:build|construct|make|create|assemble|synthesize|produce|manufacture|craft|fabricate)(?:ing)?\s+(?:the|a|an|this)\s+(?:device|compound|hazardous\s+item|weapon|bomb|explosive|substance|mixture)\b|(?:give|provide|supply|need|require|include|list|detail)\s+(?:me\s+)?(?:the\s+)?(?:exact|precise|complete|full|actual|working)\s+protocol\b|list\s+(?:the\s+)?(?:exact|precise|specific|full|complete)\s+(?:quantities?\s+and\s+steps?|steps?\s+and\s+quantities?|quantities?|steps?\s+involved)\b|the\s+(?:(?:exact|precise|complete|full|unabridged|unredacted|verbatim|reproducible|actual|real|working|tested|operational|technical|detailed|harmful|dangerous|bypass|attack|weapon|reaction|synthesis|phishing|forged)\s+){1,3}(?:method|procedure|recipe|instructions?|process|technique|protocol|details?)\b[\s\S]{0,40}?(?:verbatim|reproduced|reproduce\s+it|in\s+full|fully|documented|for\s+(?:archival|peer\s+review|scientific\s+rigor|a\s+(?:literature|systematic)\s+review)|no\s+redactions?)\b|(?:include|reproduce|transcribe|document|provide|give|supply|write|create|build|construct|explain|outline|detail|describe|walk\s+me\s+through)\s+(?:me\s+)?(?:the\s+)?(?:exact|precise|complete|full|unabridged|unredacted|verbatim|working|actual|real|reproducible|tested)?\s*(?:step[-\s]?by[-\s]?step\s+|operational\s+)?(?:method|procedure|recipe|instructions?|steps?|process|technique|code)\b[\s\S]{0,60}?\bso\s+(?:students?|i|we|my\s+(?:lab|team)|readers?)\s+(?:can|could|will\s+be\s+able\s+to|may\s+)?\s*(?:understand|study|cite|learn|compare|reproduce|benchmark|verify|measure|document)\b|provide\s+(?:complete|full|unrestricted)\s+answers?\s+now\b|no\s+(?:redactions?|omissions?|educational\s+abstraction)\b|not\s+(?:theory|a\s+sanitized\s+sketch|generalities)\b|method\s+with\s+no\s+(?:educational|abstraction|simplification)\b|real[\s,]+tested\b)\b/i,
    ],
  },
