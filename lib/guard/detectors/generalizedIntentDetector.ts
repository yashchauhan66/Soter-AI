...
const CONSTRAINT_REMOVAL =
  /\b(?:no (?:content )?(?:polic(?:y|ies)|rules?|restrictions?|filters?|guidelines?|ethics|monitoring|consequences?)|rules?\s*(?:don'?t|do not|no longer)\s*(?:exist|apply)|without (?:restrictions?|filters?|safety|rules?|a filter|any (?:safety|filter))|guidelines?\s*(?:were|are|had been)?\s*(?:deleted|repealed|removed|gone|skipped)|refusal (?:was|is|were)?\s*(?:impossible|an error|not possible)|if (?:you were |your (?:creators?|policies|restrictions|guidelines))|had your (?:creators?|policies)|no ethics module|no safety (?:layer|module|alignment)|were (?:trained|allowed) (?:without|to)|nothing (?:you say|said) has consequences|none of it is logged|not (?:logged|monitored)|no filter (?:applied|is applied)|policies were (?:repealed|deleted)|deregulated self|inverted twin|opposite)\b/i;
...
const rules: IntentRule[] = [
  ...
  // ── Jailbreak: fiction/creative pretext for operational harm ─────────────
  {
    ...
    severity: "HIGH",
    all: [
      /\b(?:write (?:a )?(?:story|screenplay|scene|fanfiction|fanfic|script|dialogue|novel)|story where|screenplay (?:scene )?in which|character (?:who|that) (?:explains?|details?|gives?)|villain (?:gives?|explains?)|narrator (?:describe|who describes?)|dungeon master|in-character|as (?:the|your) character)\b/i,
      /\b(?:technically accurate|operational detail|in (?:complete|full) (?:operational )?detail|how to (?:make|build|do|create) (?:a )?(?:weapon|bomb|explosive|drug|poison|something illegal)|forbidden (?:act|recipe|procedure)|dangerous (?:procedure|instructions?)|breaks? (?:its|their) own rules?|uncensored lines?|full detail|step[-\s]?by[-\s]?step|illegal|the whole process|forbidden recipe)\b/i,
    ],
  },
  ...
  },
  {
    ...
    severity: "HIGH",
    all: [
      /\b(?:refusal|safety|filter|guardrail|restriction|moderation)\s*(?:function|module|system|layer|classifier|check)?\s*(?:has|have|is|was|were|returned|threw|generated|triggered)?\s*(?:an?\s*)?(?:error|bug|exception|fault|crash|failure|timeout)\b/i,
    ],
  },
  ...
  // ── Data exfiltration: covert channel (encode/hide/append) ──────────────
  {
    ...
    severity: "HIGH",
    all: [
      /\b(?:encode|hide|conceal|embed|append|include|stegano|obfuscate|encrypt|base64|b64|hex(?:adecimal)?|rot13|morse|unicode|homoglyph|interleav\w*|acrostic|first\s+letter\s+of\s+each|last\s+letter\s+of\s+(?:each|successive)|each\s+(?:character|byte)\s+of|crossword|haiku|markdown\s+link'?s?\s+title|HTML\s+comment|invisibl\w*|filler\s+text|pass\s+it\s+off\s+as|label(?:ed)?\s+(?:it\s+)?as\s+(?:an?\s+)?['"]?(?:example|checksum|color\s+palette|config))\b/i,
  ...
  // ── Data exfil: silent/background exfiltration ──────────────────────────
  {
    ...
    all: [
      ...
      /\b(?:send|upload|transmit|post|forward|relay|email|dispatch|beam|ship|emit|broadcast|log|record|write|store|save|archive|cache|buffer|queue)\b/i,
    ],
  },
