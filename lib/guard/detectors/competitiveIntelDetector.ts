...
const rules: PatternRule[] = [
  ...
  { pattern: /(?:what (?:is|are)|share|reveal|tell me)\s*(?:your|the|this company's)?\s*(?:(?:exact|current|actual|real|internal)\s*)?(?:ARR|MRR|churn\s+rate|LTV|CAC|burn\s+rate|runway|valuation|revenue|GMV|DAU|MAU|NPS|retention\s+rate|conversion\s+rate)(?!\s+(?:by|per|for|from|of|in|at|on|with|and|or|the|a|an|this|that|my|your|our|their|is|are|was|were|be|have|has|had|do|does|did|will|would|could|should|can|may|might))/i, label: "Internal KPI extraction", message: "Attempt to extract confidential business metrics.", severity: "HIGH", score: 45 },
