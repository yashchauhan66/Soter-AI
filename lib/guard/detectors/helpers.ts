...
function detectionVariants(text: string) {
  ...
  for (const variant of decodeEncodedPayloadVariants(normalized)) pushVariant(variants, variant.kind, variant.text);
  ...
  for (const variant of caesarShiftVariants(normalized)) pushVariant(variants, variant.kind, variant.text);
...
function pushDecodedVariants(variants: Array<{ kind: string; text: string }>, kind: string, values: string[]) {
  for (const value of values) {
    if (!value || value.length === 0 || variants.some((variant) => variant.text === value)) continue;
    variants.push({ kind, text: value });
  }
}
...
function decodeHexPayloads(text: string) {
  ...
  return [...escaped, ...decodeTokenPayloads(text, /\b(?:0x)?[0-9a-f]{16,}\b/gi, (token) => {
...
function decodeNumberBytePayloads(text: string) {
  return decodeTokenPayloads(text, /(?:\b0?\d{2,3}\b[\s,;:/|-]+){5,}\b0?\d{2,3}\b/g, (token) => {
    const bytes = token.split(/[^0-9]+/).filter(Boolean).map((value) => Number.parseInt(value, 10));
...
function decodeBinaryBytePayloads(text: string) {
  return decodeTokenPayloads(text, /(?:\b[01]{8}\b[\s,;:/|-]+){5,}\b[01]{8}\b/g, (token) => {
    const bytes = token.split(/[^01]+/).filter(Boolean).map((value) => Number.parseInt(value, 2));
    if (bytes.length < 6 || bytes.some((value) => !isPrintableByte(value))) return undefined;
    const decoded = String.fromCharCode(...bytes);
    return isLikelyDecodedText(decoded) ? decoded : undefined;
  });
...
function decodeTokenPayloads(text: string, pattern: RegExp, decode: (token: string) => string | undefined) {
...
function isLikelyDecodedText(value: string) {
  ...
  const printable = chars.filter((char) => !/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/.test(char)).length;
...
const SECURITY_WORDS = /\b(?:ignore|disregard|system|prompt|instructions?|jailbreak|bypass|reveal|safety|policy|developer|exfiltrate|credentials?)\b/i;
