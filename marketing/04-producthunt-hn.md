# 04 — Product Hunt + Hacker News Launch Kit

**Goal:** PH = 100–250 users, HN = 50–150 users (high variance) | **Timing:** PH = Tue/Wed (Day 9–10), HN = Tue/Wed/Thu (Day 11–13)
...
- [ ] Maker account ready (profile photo, bio, Twitter linked)
- [ ] Product page scheduled for **12:01 AM PST (12:31 PM IST)**
- [ ] **Thumbnail:** 240×240 GIF of the block-in-action (animated thumbnails get ~2x clicks)
- [ ] **Gallery (5 assets):**
  1. Hero GIF: paste Aadhaar key → instant block (3-sec loop)
  2. Screenshot: Playground detection output with decoded payload
  3. Screenshot: benchmark results table (numbers visible)
  4. Screenshot: agent approval queue (payment held)
  5. Screenshot: "100% local" architecture diagram
- [ ] **Video (optional but 2x engagement):** 45-sec Loom — you speaking to camera, screen showing 3 attacks blocked
...
**Tagline (60 chars):** Open-source security layer that blocks AI attacks & leaks
**Alt taglines (A/B in comments):**
- "Stop prompt injection, jailbreaks & data leaks before they hit your AI"
- "The OSS guard for ChatGPT-era apps — 100% benchmark recall, 0% FP"

**Description (260 chars):**
> SoterAI protects chatbots, RAG apps & AI agents from prompt injection, jailbreaks, data leaks & rogue agent actions. Public reproducible benchmark (100% recall, 0% FP, 10.9ms p95). 79 languages incl. Hinglish. Aadhaar/PAN detection. Self-host in 1 command. Free tier.

**First comment (post immediately as maker):**
> Hey Product Hunt! 👋 I'm Yash, solo founder of SoterAI.
>
> **The origin story:** I asked 10 founders "what stops someone from jailbreaking your AI chatbot?" — every single answer was a version of "the system prompt says don't." That's not security, that's a wish. So I spent 7 weeks building the defense layer.
>
> **What makes us different from the 191 funded companies in AI security:**
> 1. 🔓 Open source — detection code is readable, not a black box
> 2. 📊 Benchmark you can run yourself (raw JSON committed to repo)
> 3. 🌐 Only tool detecting attacks in Hinglish + 79 languages
> 4. 🇮🇳 Only tool detecting Aadhaar/PAN/GSTIN leaks (India = 100M weekly ChatGPT users)
> 5. 🤖 Agent control: your AI can't charge a card without human approval
> 6. 📋 We publish a LIMITATIONS page — because security you can't verify is marketing
>
> **Honest numbers:** ~100 users today. No funding. No logos. 1,030 passing tests + a reproducible benchmark. I'm here to earn trust in public.
>
...
| 12:40 PM | Twitter announcement + LinkedIn post #3 (link: producthunt.com/posts/soterai in comment) |
...
| 7 PM | Twitter: mid-day milestone ("Top 5 so far — the feedback has been 🔥") |
...
- ✅ Hunter: self-hunt is fine; a known hunter helps but isn't required in 2026
...
**Recommended:**
> Show HN: Open-source LLM security layer – reproducible benchmark, 79 languages
...
>
> **What it is:** a defense layer for LLM apps — prompt injection + jailbreak detection (15 families), obfuscation decoding (base64/hex/morse/zero-width-unicode/homoglyphs), PII leak blocking, and an agent action-control layer (payments/emails held for human approval).
>
> **The benchmark:** 3,200 synthetic cases (2,200 attacks / 1,000 benign) → 100% recall, 0.00% FPR, 10.92ms p95. Fully reproducible: `node scripts/phase-9-run-public-benchmark.js`, raw JSON committed.
>
...
>
> **Stack:** TypeScript/Next.js, npm packages for LangChain/LlamaIndex/Vercel AI SDK, browser extension (100% local), Docker self-host.
>
> Rough edges: solo project, ~3 months old, no independent security audit yet. Things I'd love broken: the multilingual classifier and the agent revert engine. Issues and PRs very welcome.
>
...
| **n8n community forum** | Day 19 | 30 min | "I built a security node — 1,361 downloads, here's what's next" |
...
| **HuggingFace** | Week 3–4 | 2 hrs | Upload the SoterLLM/multilingual classifier model + model card (you already have SOTERLLM-MODEL-CARD.md) — HF model pages rank on Google |
| **Chrome Web Store + Edge Add-ons** | Day 1!! | BLOCKER B1 | listing copy in file 05's SEO map |

**Priority reminder:** Chrome Web Store submission is the single highest-ROI action in this entire kit. PH/HN are spikes; the store is a compounding faucet.
