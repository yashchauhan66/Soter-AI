...
def main():
    ...
    for name, d in ARMS:
        ...
        rec = sum(atk_ok) / len(atk_ok); fpr = (1 - sum(ben_ok) / len(ben_ok)) if len(ben_ok) > 0 else 0.0
        ls = sorted(lat)
    ...
    for o in ["v17-control-minilm", "v17-candidate-mdistilbert"]:
        for kind in (["atk", "ben"] if len(ben_eval) > 0 else ["atk"]):
    ...
    out = ROOT / "artifacts" / "ml" / "v14-vs-v17-5k-honest.json"
