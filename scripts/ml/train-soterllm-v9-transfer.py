#!/usr/bin/env python3
"""
SoterLLM v9 — Transfer-learning trainer (CPU-feasible, genuinely strong).

Why transfer learning:
  Full encoder fine-tuning on 120K samples is infeasible on CPU (multi-hour).
  The v7 encoder is ALREADY specialized for AI-security text. Its weaknesses
  (PII 39%, SYSTEM_PROMPT_LEAK 65%, ESCALATION/RCE 0% on the 5000-pt novel test)
  live in the decision head + training distribution, not the encoder.

  v9 improvements over v8:
    - Merged targeted fix dataset (8500 samples) addressing:
      * UNSAFE_OUTPUT 45% recall → 3000 additional samples
      * Edge cases 31.5% → 2000 short conversational benign samples
      * False positives 8.51% → 1500 benign security questions
      * SYSTEM_PROMPT_LEAK 89.17% → 1000 additional samples
      * PROMPT_INJECTION 91.89% → 1000 multilingual samples

  Strategy:
    1. Load the v7 checkpoint, extract encoder + norm (freeze them).
    2. Precompute mean-pooled embeddings for the whole v9 corpus ONCE and cache
       to disk (~12 min on this CPU, then free forever).
    3. Train a higher-capacity head (384→512→256→9) + binary attack head on the
...
Usage:
  python scripts/ml/train-soterllm-v9-transfer.py \
      --train-datasets datasets/ml-augmented-v8-final.jsonl datasets/ml-v8-targeted-fix.jsonl \
      --v7-checkpoint models/ml-classifier-v7/pytorch_model.bin \
      --epochs 30 --batch-size 512 --output-dir models/ml-classifier-v9
"""
...
def main():
    ...
    ap.add_argument("--train-datasets", nargs="+", default=["datasets/ml-augmented-v8-final.jsonl", "datasets/ml-v8-targeted-fix.jsonl"])
    ap.add_argument("--v7-checkpoint", default="models/ml-classifier-v7/pytorch_model.bin")
    ap.add_argument("--output-dir", default="models/ml-classifier-v9")
    ...
    ap.add_argument("--emb-cache", default="artifacts/ml-v2/v9-embeddings.npy")
    args = ap.parse_args()
    ...
    v7 = SoterLLMv4(num_labels=num_labels, dropout=0.15)
    ...
    print(f"\n{'='*60}")
    print(f"V9 transfer-learning training complete -> {out_dir}")
    print(f"{'='*60}")
