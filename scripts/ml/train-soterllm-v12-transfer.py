...
def group_aware_split(groups, val_frac=0.12, cal_frac=0.08, seed=42):
    # sorted(), NOT list(set()). Python randomizes str hashing per process unless
    # PYTHONHASHSEED is pinned, so `list(set(groups))` yields a DIFFERENT order in
    # every run and the seeded shuffle below then produces a different partition.
    #
    # This is not theoretical. v12 shipped with dataset_manifest.json recording
    # train/cal/val = 115379/11561/17392; re-running this exact loader on the exact
    # same six dataset files (same 144,332 rows, same 126,126 groups) reproduced
    # 115259/11676/17397 instead. The rows and grouping were byte-identical — only
    # the set-iteration order differed. Consequence: v12's calibration split is
    # PERMANENTLY unrecoverable, so the OOD entropy budgets it never wrote
    # (lib/ml/calibration.ts labelSpaceUncertain) cannot be fitted on the split
    # they belong to. See scripts/ml/fit-missing-calibration-entropy.py.
    #
    # sorted() makes the partition a pure function of (group set, seed), so from
    # v13 on the split is recoverable from the manifest alone.
...
def main():
    ...
    ap.add_argument("--train-datasets", nargs="+", default=["datasets/ml-augmented-v8-final.jsonl", "datasets/ml-v8-targeted-fix.jsonl", "datasets/ml-v10-advanced-attacks.jsonl", "datasets/ml-v10-targeted-fix.jsonl", "datasets/ml-v11-weak-fix.jsonl", "artifacts/ml-v2/v12-toxicity-fix.jsonl"])
    ap.add_argument("--v7-checkpoint", default="models/ml-classifier-v7/pytorch_model.bin")
    ap.add_argument("--output-dir", default="models/ml-classifier-v12")
    ...
    ap.add_argument("--emb-cache", default="artifacts/ml-v2/v12-final-embeddings.npy")
    args = ap.parse_args()
    ...
    # Entropy budgets. These are NOT optional extras: lib/ml/calibration.ts reads
    # them by name, and a missing field silently DISABLES the gate that reads it —
    # `labelSpaceUncertain` has no env override and returns false whenever
    # entropy_p95 is absent, which switches off the truncated-view OOD check in
    # onnxBackend.ts (the check that stopped a benign 40-clause contract from
    # escalating as DATA_EXFILTRATION_ATTEMPT at P(attack) 0.9971). v8..v12 all
    # shipped without them because this dict only had the max_prob fields.
    #
    # Two budgets, deliberately different questions:
    #   entropy_p95        — spread across ALL labels; only meaningful on a partial
    #                        view of the input. Used by labelSpaceUncertain.
    #   binary_entropy_p95 — spread on attack-vs-safe alone; what shouldAbstain
    #                        spends. Max possible is ln 2 = 0.6931, so anything at
    #                        or above that disables entropy-driven abstention.
    # Both fitted on the calibration split, same rows as the thresholds above.
    cal_p_attack = 1.0 - cal_probs[:, 0]          # index 0 is SAFE in ALL_LABELS
    with np.errstate(divide="ignore", invalid="ignore"):
        cal_label_entropy = -(cal_probs * np.log(np.clip(cal_probs, 1e-12, None))).sum(axis=1)
        _p = np.clip(cal_p_attack, 1e-12, 1 - 1e-12)
        cal_binary_entropy = -(_p * np.log(_p) + (1 - _p) * np.log(1 - _p))
        cal_binary_entropy[(cal_p_attack <= 1e-12) | (cal_p_attack >= 1 - 1e-12)] = 0.0
    ...
    calibration = {
        ...
        "ood": {"max_prob_p05": float(np.percentile(cal_probs.max(axis=1), 5)),
                ...
                "entropy_mean": float(cal_label_entropy.mean()),
                "entropy_p95": float(np.percentile(cal_label_entropy, 95)),
                "binary_entropy_p95": float(np.percentile(cal_binary_entropy, 95))},
    ...
    print(f"\n{'='*60}")
    print(f"V12 transfer-learning training complete -> {out_dir}")
    print(f"{'='*60}")
