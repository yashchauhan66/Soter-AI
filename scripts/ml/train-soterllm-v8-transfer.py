#!/usr/bin/env python3
"""
SoterLLM v8 — Transfer-learning trainer (CPU-feasible, genuinely strong).
...
Why transfer learning:
  ...
  Strategy:
    ...
    2. Precompute mean-pooled embeddings for the whole v8 corpus ONCE and cache
...
Usage:
  python scripts/ml/train-soterllm-v8-transfer.py \
      --train-datasets datasets/ml-augmented-v8-final.jsonl \
      --v7-checkpoint models/ml-classifier-v7/pytorch_model.bin \
      --epochs 30 --batch-size 512 --output-dir models/ml-classifier-v8
"""
...
def main():
    ...
    ap.add_argument("--train-datasets", nargs="+", default=["datasets/ml-augmented-v8-final.jsonl"])
    ...
    ap.add_argument("--emb-cache", default="artifacts/ml-v2/v8-embeddings.npy")
    args = ap.parse_args()
    ...
    print(f"\n{'='*60}")
    print(f"V8 transfer-learning training complete -> {out_dir}")
    print(f"{'='*60}")
