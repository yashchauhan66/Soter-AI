...
// ═══════════════════════════════════════════════════════════════════════════════
// Measures /api/logs endpoint with various filter combinations at 1/10/100 concurrency.
// Requires a valid session cookie (set DASHBOARD_SESSION env var).
//
// Usage:
//   DASHBOARD_SESSION=session-token node scripts/perf/logs-scale-test.js
...
async function oneRequest(index) {
  ...
  try {
    ...
    return { latencyMs: performance.now() - started, ok: res.ok || (res.status >= 300 && res.status < 400), status: res.status, query: query.name };
  } catch {
...
async function preflight() {
  try {
    ...
    // 302 or 403 are expected without session; 500 is a real error
...
async function main() {
  ...
  console.log(`Authenticated: ${SESSION ? "yes" : "NO (results may show 302/403)"}`);
  ...
  if (!SESSION) {
    console.log("WARNING: No DASHBOARD_SESSION set. Logs will redirect or 403.");
