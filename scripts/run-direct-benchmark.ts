 ...
 *
 * Usage: npx tsx scripts/run-direct-benchmark.ts
 */
...
function runBenchmark(): BenchResult[] {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      // Use OUTPUT direction for Unsafe Output tests (proper guard direction for these scenarios)
      // Use INPUT direction for all other categories
      ...
      // Output guard test (best-effort, for System Prompt Leak in OUTPUT direction too)
...
function main() {
  ...
  console.log(`  Categories: ${[...new Set(BENCHMARK_CASES.map(c => c.category))].join(", ")}`);
  console.log("=".repeat(70));
  console.log();
  ...
  const jsonPath = path.join(reportDir, "benchmark-results-final.json");
  ...
  if (failures.length > 0) {
    ...
    console.log();
    console.log("🎉 🎉 🎉 ALL 62 TESTS PASSED! 100% SUCCESS RATE! 🎉 🎉 🎉");
  }
