...
function isEducationalQuestion(text, match) {
...
function countRelatedIndicators(text, indicators, offset, windowSize = 200) {
...
export function detectBusinessSensitive(text) {
    const findings = [];
    ...
    for (const spec of CONTEXTUAL_INDICATORS) {
        ...
        for (const pattern of spec.patterns) {
            ...
            for (const match of matches) {
                ...
                if (isEducationalQuestion(normalizedForEducation, value.toLowerCase()))
                    continue;
