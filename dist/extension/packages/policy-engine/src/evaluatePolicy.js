import { ACTION_PRECEDENCE, maxAction, maxSeverity } from "./actions.js";
import { severityForRiskScore, thresholdAction } from "./riskScore.js";
...
function ruleMatches(rule, input) {
    if (rule.destinations?.length && !rule.destinations.some((domain) => domainMatches(input.destinationDomain, domain)))
        return false;
    if (rule.destinationTypes?.length && !rule.destinationTypes.includes(input.destinationType))
        return false;
    if (rule.departments?.length && !rule.departments.some((value) => value.toLowerCase() === "all") && (!input.department || !includesCaseInsensitive(rule.departments, input.department)))
        return false;
    if (rule.roles?.length && !rule.roles.some((value) => value.toLowerCase() === "all") && (!input.role || !includesCaseInsensitive(rule.roles, input.role)))
        return false;
    if (rule.detectedDataTypes?.length && !rule.detectedDataTypes.some((type) => input.detectedDataTypes.includes(type)))
        return false;
    if (typeof rule.minRiskScore === "number" && input.riskScore < rule.minRiskScore)
        return false;
    if (rule.intent?.length && (!input.intent || !includesCaseInsensitive(rule.intent, input.intent)))
        return false;
    return true;
}
function domainMatches(actual, pattern) {
...
function severityForAction(action) {
    if (action === "block")
        return "critical";
    if (action === "require_approval" || action === "require_justification")
        return "high";
    if (action === "redact" || action === "rewrite")
        return "medium";
    if (action === "warn")
        return "low";
    if (action === "log_only")
...
function userMessageFor(action, override) {
    if (override)
        return override;
    switch (action) {
        case "block":
...
export function redactByDataTypes(text, detectedDataTypes) {
    let output = text;
    const replacements = [
    ...
    if (detectedDataTypes.includes("credit_card"))
        output = output.replace(/\b(?:\d[ -]*?){13,19}\b/g, "[REDACTED_CARD]");
 ...
 */
export function rewriteSafePrompt(redactedText, detectedDataTypes, action) {
    if (!detectedDataTypes.length || ACTION_PRECEDENCE[action] < ACTION_PRECEDENCE.redact)
        return redactedText;
