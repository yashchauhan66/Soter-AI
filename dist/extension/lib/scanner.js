import { scanText } from "../packages/detectors/src/index.js";
import { evaluatePolicy } from "../packages/policy-engine/src/evaluatePolicy.js";
import { auditSafePreview, redactSensitiveText } from "./redaction.js";
import { rewritePromptSafely } from "./rewrite.js";
import { matchAIDestination } from "../packages/shared/src/ai-destinations.js";
export function destinationTypeForUrl(url, state) {
...
export function scanPrompt(text, url, state, eventType = "scan") {
    ...
    if (!policy)
        throw new Error("Soter policy cache is not initialized.");
...
export function emergencyLockdownAction(input) {
    ...
    if (!lockdown?.enabled)
        return null;
    if (input.eventType === "file_upload" && lockdown.blockAllFileUploads)
        return "block";
    if (lockdown.allowOnlyEnterpriseDestinations && !["enterprise_ai", "internal"].includes(input.destinationType))
        return "block";
    if (input.detectedDataTypes.some((type) => lockdown.blockedDataTypes.includes(type)))
        return "block";
    if (input.detectedDataTypes.some((type) => lockdown.requireApprovalDataTypes.includes(type)))
        return "require_approval";
    return null;
}
export function shouldPreventSubmit(action) {
    return action === "block" || action === "require_approval" || action === "require_justification";
}
export function eventName(eventType) {
    return eventType === "submit" ? "submit" : eventType;
}
function detectCustomPolicyMatches(text, policy) {
    const custom = policy;
    const matches = [];
    const normalized = text.toLowerCase();
    if (custom?.customDetectors?.keywords?.some((keyword) => keyword && normalized.includes(keyword.toLowerCase())))
        matches.push("custom_keyword");
