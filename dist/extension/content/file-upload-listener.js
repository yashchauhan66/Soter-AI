...
export function installFileUploadListener() {
    document.addEventListener("change", async (event) => {
        ...
        if (!response.ok || (!response.result.hasFindings && !names.some((name) => SENSITIVE_FILE.test(name))))
            return;
        if (["block", "require_approval", "require_justification"].includes(response.result.action))
            input.value = "";
        showSoterOverlay({
...
function sendFileScan(text) {
    return new Promise((resolve) => chrome.runtime.sendMessage({ type: "SOTER_SCAN_TEXT", text, url: location.href, eventType: "file_upload" }, (response) => resolve(response ?? { ok: false, message: chrome.runtime.lastError?.message ?? "No response." })));
}
