import { enrollmentMarkup, enrollmentStatusLabel, escapeHtml, wireEnrollment } from "../lib/enrollment-ui.js";
export function renderSidePanel(root, state) {
    ...
    root.querySelector("[data-copy-safe]")?.addEventListener("click", () => { if (latest)
        void navigator.clipboard.writeText(latest.rewrittenSafeText || latest.redactedText); });
    root.querySelector("[data-request-approval]")?.addEventListener("click", () => { if (latest)
        chrome.runtime.sendMessage({ type: "SOTER_REQUEST_APPROVAL", text: latest.redactedText, url: location.href }); });
}
function latestScanSection(latest) {
