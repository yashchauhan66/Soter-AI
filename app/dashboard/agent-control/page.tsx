  ...
  return;
      ...
      <section className="card p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><h2 className="text-lg font-semibold">Human approval queue</h2><p className="mt-1 text-sm text-slate-200">A single queue for firewall holds and transaction escrow. Payloads shown here are already redacted.</p></div>
          <span className="rounded-full bg-yellow-400/10 px-3 py-1 text-xs font-semibold text-yellow-300">{pending} pending</span>
        </div>
        {!canManage && <p className="mt-4 rounded-lg border border-slate-800 bg-slate-950/50 p-3 text-sm text-slate-200">Viewer mode: policy management permission is required to approve or deny actions.</p>}
        <div className="mt-4 grid gap-4 xl:grid-cols-2">
          {approvals.map((item) => (
            <article className="rounded-xl border border-yellow-400/20 bg-yellow-400/5 p-4" key={item.id}>
              <div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{item.tool ?? "agent.action"} / {item.action ?? "approval"}</p><p className="mt-1 text-sm text-slate-200">{item.reason}</p></div><RiskLevel level={item.riskLevel ?? "HIGH"} /></div>
              <p className="mt-2 text-xs text-slate-300">Target: {item.target ?? "unknown"} · expires {item.expiresAt.toLocaleString()}</p>
          ...
          {escrow.map((item) => (
            <article className="rounded-xl border border-orange-400/20 bg-orange-400/5 p-4" key={item.id}>
              <div className="flex items-start justify-between gap-3"><div><p className="font-semibold">Escrow · {item.tool} / {item.action}</p><p className="mt-1 text-sm text-slate-200">Held before execution against {item.target ?? "unknown target"}.</p></div><RiskLevel level={item.riskLevel} /></div>
              <p className="mt-2 text-xs text-slate-300">Expires {item.expiresAt.toLocaleString()}</p>
          ...
          {pending === 0 && <p className="text-sm text-slate-300">No agent actions are waiting for human review.</p>}
        </div>
      </section>

      <section className="card overflow-x-auto p-5">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-semibold">Reversibility ledger</h2><p className="mt-1 text-sm text-slate-200">A rollback request stages the stored compensating action for an authorized connector; it does not falsely mark an external change as reversed.</p></div><Link className="text-sm text-cyan hover:underline" href={`/dashboard/forensics?project=${project.id}`}>Open forensics</Link></div>
        <table className="mt-4 w-full min-w-[1100px] text-left text-sm">
          <thead className="text-xs uppercase text-slate-300"><tr><th className="py-2">Window</th><th>Risk</th><th>Tool action</th><th>Target</th><th>Policy</th><th>Recorded</th><th>Operator action</th></tr></thead>
          <tbody className="divide-y divide-slate-800">
            {ledger.map((item) => { const window = rollbackWindowState(item); return <tr key={item.id}><td className="py-3"><StatusBadge value={window} /></td><td><RiskLevel level={item.riskLevel} /></td><td><p className="font-mono text-xs">{item.tool}</p><p>{item.action}</p></td><td className="max-w-[220px] truncate text-slate-200">{item.targetRedacted ?? "-"}</td><td><StatusBadge value={item.decision} /></td><td className="text-xs text-slate-200">{item.createdAt.toLocaleString()}</td><td>{window === "AVAILABLE" && canManage ? <ConfirmableForm action={stageDashboardRollback} className="flex min-w-[290px] gap-2" getConfirmMessage={() => "Stage this rollback? This will prepare the agent for rollback."}><input type="hidden" name="projectId" value={project.id} /><input type="hidden" name="ledgerId" value={item.id} /><input className="input min-w-0 flex-1 text-xs" name="reason" minLength={8} maxLength={500} required placeholder="Incident or rollback reason" /><button className="button-secondary whitespace-nowrap">Stage rollback</button></ConfirmableForm> : <span className="text-xs text-slate-300">{item.irreversibleReason ?? item.summary}</span>}</td></tr>; })}
            {ledger.length === 0 && <tr><td className="py-6 text-slate-300" colSpan={7}>No ledger entries yet. Send agent actions to the action-ledger API to classify reversibility.</td></tr>}
          </tbody>
        </table>
      </section>

      <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="card overflow-x-auto p-5"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-semibold">Live action audit</h2><Link className="text-sm text-cyan hover:underline" href={`/dashboard/logs?project=${project.id}`}>All logs</Link></div><table className="mt-4 w-full min-w-[760px] text-left text-sm"><thead className="text-xs uppercase text-slate-300"><tr><th className="py-2">Decision</th><th>Risk</th><th>Tool / action</th><th>Destination</th><th>Reason</th><th>When</th></tr></thead><tbody className="divide-y divide-slate-800">{logs.slice(0, 20).map((item) => <tr key={item.id}><td className="py-3"><StatusBadge value={item.decision} /></td><td><RiskLevel level={item.riskLevel} /></td><td><span className="font-mono text-xs">{item.tool}</span> / {item.action}</td><td>{item.destination}</td><td className="max-w-[260px] truncate text-slate-200">{item.reason}</td><td className="text-xs text-slate-300">{item.createdAt.toLocaleString()}</td></tr>)}</tbody></table>{logs.length === 0 && <p className="py-5 text-sm text-slate-300">No agent actions logged yet.</p>}</section>
        <section className="card p-5"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-semibold">Compliance posture</h2><StatusBadge value={assurance.overallStatus} /></div><p className="mt-2 text-sm text-slate-200">{metrics.freshEvidence} fresh evidence items · {assurance.summary.passing}/{assurance.summary.totalControls} controls passing</p><div className="mt-4 space-y-2">{assurance.controls.map((control) => <div className="flex items-center justify-between gap-3 rounded-lg border border-slate-800 p-3 text-sm" key={control.id}><div><p className="font-medium">{control.name}</p><p className="text-xs text-slate-300">{control.id} · {control.evidenceIds.length} fresh items</p></div><StatusBadge value={control.status} /></div>)}</div><Link className="mt-4 inline-block text-sm text-cyan hover:underline" href={`/dashboard/evidence-vault?project=${project.id}`}>Open evidence vault →</Link></section>
      </div>

      <section className="card p-5"><h2 className="text-lg font-semibold">Operator audit trail</h2><p className="mt-1 text-sm text-slate-200">Human rollback decisions are attributed to an operator and retained separately from agent-generated logs.</p><div className="mt-4 space-y-2">{operatorAudits.map((audit) => <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-800 p-3 text-sm" key={audit.id}><div><StatusBadge value={audit.action} /><span className="ml-2 text-slate-200">Actor {audit.actorUserId ?? "system"}</span></div><span className="text-xs text-slate-300">{audit.createdAt.toLocaleString()}</span></div>)}{operatorAudits.length === 0 && <p className="text-sm text-slate-300">No human rollback decisions recorded yet.</p>}</div></section>
    </div>
  );
}
