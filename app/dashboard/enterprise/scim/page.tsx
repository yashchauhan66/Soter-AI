    ...
    <div>
      ...
      <section className="mt-6 grid gap-5 lg:grid-cols-2">
        <div className="card p-5">
          ...
          <div className="mt-4 space-y-3 text-sm">
            {tokens.length === 0 ? <p className="text-slate-200">No SCIM tokens generated.</p> : tokens.map((token) => (
              <div className="rounded-lg border border-slate-800 p-3" key={token.id}>
                <p className="font-medium">{token.name}</p>
                <p className="text-slate-200">{token.tokenPreview} · {token.revokedAt ? "revoked" : "active"} · last used {token.lastUsedAt?.toISOString() ?? "never"}</p>
              </div>
            ))}
          </div>
        </div>
        <div className="card p-5">
          ...
          <div className="mt-4 space-y-3 text-sm">
            {groups.length === 0 ? <p className="text-slate-200">Groups provisioned by the IdP will appear here.</p> : groups.map((group) => (
              <div className="rounded-lg border border-slate-800 p-3" key={group.id}>
                <p className="font-medium">{group.displayName}</p>
                <p className="text-slate-200">Maps to {group.role} · {Array.isArray(group.members) ? group.members.length : 0} member references</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mt-6 card p-5">
        ...
        <div className="mt-4 space-y-2 text-sm">
          {events.length === 0 ? <p className="text-slate-200">No SCIM audit events yet.</p> : events.map((event) => (
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-800 py-2" key={event.id}>
              <span>{event.action}</span>
              <span className="text-slate-300">{event.createdAt.toISOString()}</span>
            </div>
          ))}
        </div>
      </section>
