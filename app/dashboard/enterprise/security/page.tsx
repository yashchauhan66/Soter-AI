    ...
    <div>
      ...
      <section className="mt-6 grid gap-5 lg:grid-cols-3">
        <div className="card p-5">
          ...
          <p className="mt-3 text-sm text-slate-200">IP allowlist is {organization?.ipAllowlistEnabled ? "enabled" : "disabled"}.</p>
          <div className="mt-4 space-y-2 text-sm">{allowlist.map((entry) => <p key={entry.id}>{entry.cidr} <span className="text-slate-300">{entry.label ?? ""}</span></p>)}</div>
        </div>
        <div className="card p-5">
          <h2 className="font-semibold">Keys and quotas</h2>
          <dl className="mt-4 space-y-2 text-sm">
            <div className="flex justify-between gap-3"><dt className="text-slate-300">Rotation policy</dt><dd>{organization?.apiKeyRotationDays ? `${organization.apiKeyRotationDays} days` : "Not set"}</dd></div>
            <div className="flex justify-between gap-3"><dt className="text-slate-300">Quota override</dt><dd>{organization?.quotaOverride ?? "None"}</dd></div>
          </dl>
        </div>
        <div className="card p-5">
          <h2 className="font-semibold">Tenant state</h2>
          <p className="mt-3 text-sm text-slate-200">{organization?.disabled ? `Disabled: ${organization.disabledReason ?? "No reason recorded"}` : "Organization enabled"}</p>
        </div>
      </section>
      <section className="mt-6 card p-5">
        <h2 className="font-semibold">Sessions</h2>
        <div className="mt-4 space-y-2 text-sm">
          {sessions.length === 0 ? <p className="text-slate-200">No tracked sessions.</p> : sessions.map((session) => (
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-800 py-2" key={session.id}>
              ...
              <span className="text-slate-300">{session.revokedAt ? "revoked" : "active"}</span>
            </div>
          ))}
        </div>
      </section>
