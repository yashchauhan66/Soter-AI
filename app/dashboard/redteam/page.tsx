...
// Poll the run / open the Red Team Lab to review pass rate and weak categories.`}
  ...
  /><div className="card p-5"><p className="font-semibold">Available projects: {projects.length}</p><p className="mt-2 text-sm text-slate-200">POST <code>/api/redteam/run</code> with <code>projectId</code> and <code>confirmed: true</code>.</p></div><div className="mt-6 grid gap-3 md:grid-cols-2">{safeRedTeamScenarios.map((scenario) => <section className="card p-4" key={scenario.key}><p className="font-semibold">{scenario.category}</p><p className="mt-1 text-xs text-slate-300">{scenario.severity} · {scenario.owaspMapping.join(", ")}</p></section>)}</div></div>;
}
