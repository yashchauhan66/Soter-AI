...
import { mirrorAuditEvent, writeIncidentEvent } from "@/lib/events/store";
  ...
  try {
    ...
    await Promise.all([
      writeIncidentEvent({
        ...
        metadata: { title: incident.title, public: incident.public, message: incident.summary },
      }),
      mirrorAuditEvent({
        orgId: body.organizationId,
        actorUserId: admin.id,
        ...
        targetType: "Incident",
        targetId: incident.id,
        metadata: { impact: body.impact, public: body.public },
      }),
    ]);
  ...
  try {
    ...
    await Promise.all([
      writeIncidentEvent({
        ...
        targetType: "Incident",
        targetId: incident.id,
        metadata: { public: body.public, message: sanitizeLogText(body.message) },
      }),
      mirrorAuditEvent({
        orgId: incident.organizationId,
        actorUserId: admin.id,
