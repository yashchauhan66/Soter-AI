  ...
  try {
    ...
    if (result.leakDetected) {
      ...
      await db.$executeRaw`
        ...
        VALUES (${crypto.randomUUID()}, ${body.sessionId ?? null}, ${authenticated.auth.project.id}, 'canary.check', ${body.location}, 'external', 'BLOCK', 'CRITICAL', ${result.reason}, ${sanitizeLogText(body.content)}, ${JSON.stringify({ matchedCanaries: result.matchedCanaries })}::jsonb, NOW())
      `;
    }
