...
export async function POST(request: Request) {
  try {
    ...
    return jsonResponse({ ok: true, state: await setEmergencyLockdown({ ...body, enabled: false, adminId: admin.id }) });
  } catch (error) {
    return apiError(error, "Emergency lockdown could not be disabled.");
  }
}
