...
export async function POST(request: Request) {
  try {
    ...
    const feedback = await db.detectionFeedback.upsert({ where: { guardLogId_createdById: { guardLogId: log.id, createdById: access.user.id } }, create: { organizationId: access.org.id, projectId: log.projectId, guardLogId: log.id, feedback: body.feedback, note: body.note, createdById: access.user.id }, update: { feedback: body.feedback, note: body.note } });
