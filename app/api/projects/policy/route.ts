...
const upsertSchema = z.object({
  ...
  bareInjectionHandling: z.enum(['REWRITE', 'HUMAN_REVIEW', 'BLOCK']).optional(),
...
export async function GET(request: Request) {
  try {
    ...
    if (!projectId)
      return jsonResponse({ error: true, message: 'projectId required.' }, { status: 400 });
    ...
    return apiError(error, 'Policy could not be loaded.');
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    const data = {
      ...
      bareInjectionHandling: body.bareInjectionHandling ?? 'REWRITE',
    ...
    await emitSecurityEvent({
      ...
      riskTypes: [],
      action: 'UPDATED',
      source: 'api.projects.policy',
      metadata: {
        mode: policy.mode,
        citationRequired: policy.citationRequired,
        minSourceCount: policy.minSourceCount,
      },
    });
    return jsonResponse(policy);
  } catch (error) {
    return apiError(error, 'Policy could not be saved.');
  }
}
