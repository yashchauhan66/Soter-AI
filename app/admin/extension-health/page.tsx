      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="card p-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-slate-200">Versions</span>
          </div>
        ...
        <div className="card p-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-slate-200">Organizations</span>
          </div>
            ...
            {deviceAgents.map((agent) => {
              ...
              return (
                <tr key={agent.id}>
                  ...
                  <td className="px-4 py-3 text-slate-200">{agent.organization?.name ?? agent.organizationId}</td>
                  <td className="px-4 py-3 text-slate-200">{agent.version}</td>
                  <td className="px-4 py-3 text-slate-200">{agent.platform}</td>
                  ...
                  <td className="px-4 py-3 text-xs text-slate-300">{agent.policyVersion ?? "—"}</td>
                  <td className="px-4 py-3 text-slate-300">
                    {agent.lastHeartbeatAt ? new Date(agent.lastHeartbeatAt).toLocaleString() : "Never"}
                  </td>
                  <td className="px-4 py-3 text-slate-300">{agent.activeDestination ?? "—"}</td>
                </tr>
              );
            })}
          </tbody>
