...
export default async function AdminSupplyChainPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-3 text-slate-200">Internal Preview counts for AI supply-chain findings. Complete AI BOM lifecycle, review, and export workflows are tracked separately.</p>
      <div className="mt-7 grid gap-4 sm:grid-cols-4">
        {rows.length ? rows.map((row) => (
          <section className="card p-5" key={row.severity}>
            <p className="text-sm text-slate-200">{row.severity}</p>
            <p className="mt-2 text-2xl font-bold">{Number(row.count)}</p>
          </section>
        )) : <p className="text-slate-300">No findings recorded.</p>}
      </div>
