...
export default async function AbusePage() {
  ...
  return (
    <div>
      ...
      <p className="mt-3 text-slate-200">Internal Preview for abuse signals, cost anomalies, spike detection, and throttling evidence. Route-wide production enforcement is tracked separately.</p>
      <div className="mt-7 grid gap-4 sm:grid-cols-3">
        {[["Abuse signals", signals], ["Usage anomalies", anomalies], ["Throttle events", throttles]].map(([label, value]) => <section className="card p-5" key={String(label)}><p className="text-sm text-slate-200">{label}</p><p className="mt-2 text-2xl font-bold">{String(value)}</p></section>)}
      </div>
