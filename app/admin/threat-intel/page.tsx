...
export default async function ThreatIntelPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-3 text-slate-200">Internal Preview for rule validation and lifecycle planning. Remote feeds, approval workflow, promotion, and rollback require authorized setup before production use.</p>
      <div className="mt-7 grid gap-4 sm:grid-cols-3">
        {[["Sources", sources], ["Patterns", patterns], ["Rule versions", ruleVersions]].map(([label, value]) => <section className="card p-5" key={String(label)}><p className="text-sm text-slate-200">{label}</p><p className="mt-2 text-2xl font-bold">{String(value)}</p></section>)}
      </div>
