...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <main className="container-page py-10">
      ...
      <nav className="mb-7 flex flex-wrap gap-2 text-sm">{[["Overview", "/admin"], ["Organizations", "/admin/organizations"], ["Projects", "/admin/projects"], ["KMS", "/admin/kms"], ["Classifiers", "/admin/classifier-evals"], ["ML registry", "/admin/ml"], ["ML datasets", "/admin/ml/datasets"], ["ML evaluations", "/admin/ml/evaluations"], ["ML review", "/admin/ml/review"], ["ML deployments", "/admin/ml/deployments"], ["Red team", "/admin/redteam"], ["SIEM", "/admin/siem"], ["System health", "/admin/system-health"]].map(([label, href]) => <Link className="rounded-lg border border-slate-800 px-3 py-2 text-slate-300 hover:border-cyan-500/50" href={href} key={href}>{label}</Link>)}</nav>
      {children}
    </main>
  );
}
