...
async function loadProductionMetrics() {
  ...
  return await Promise.all([
    ...
    db.webhookDelivery.count({ where: { status: { in: ["FAILED", "DEAD_LETTER"] }, createdAt: { gte: since } } }),
    db.webhookDelivery.count({ where: { createdAt: { gte: since } } }),
    db.paymentEvent.count({
      where: { eventType: { in: ["payment.failed", "subscription.halted"] }, receivedAt: { gte: since } },
    }),
    db.guardLog.count({ where: { createdAt: { gte: since } } }),
    db.guardLog.count({ where: { action: "BLOCK", createdAt: { gte: since } } }),
    checkSecretStoreHealth(),
    getVectorProvider()
      .then((provider) => provider.healthCheck())
      .catch(() => ({ healthy: false, provider: "unknown", message: "Unavailable", latencyMs: 0, configured: false })),
    db.$queryRaw`SELECT 1`
      .then(() => "healthy")
      .catch(() => "unavailable"),
    getRedis()
      .get<number>("crg:production:health")
...
export default async function ProductionPage() {
  ...
  const cards = [
    ...
    ["Guard API p95", metrics.guardApiP95 ?? "No samples", "ms"],
    ["Dashboard p95", metrics.dashboardP95 ?? "No samples", "ms"],
    ["Error rate", metrics.requestCount ? formatPercent(metrics.errorCount, metrics.requestCount) : "No samples", ""],
    ["Worker backlog", pendingJobs, "jobs"],
    ["Failed jobs", failedJobs, "24h"],
    ["Webhook failure rate", formatPercent(failedWebhooks, totalWebhooks), ""],
    ["Billing failures", billingFailures, "24h"],
    ["Detection volume", detectionVolume, "24h"],
    ["Blocked risk volume", blockedVolume, "24h"],
    ["Redis health", redisHealth, ""],
    ["DB health", dbHealth, ""],
    ["Vector provider", vector.healthy ? `healthy (${vector.latencyMs}ms)` : vector.message, ""],
    ["KMS health", kms.healthy ? "healthy" : kms.message, ""],
  ];
