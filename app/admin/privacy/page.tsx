...
export default async function AdminPrivacyPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-3 text-slate-200">Readiness counts and drafting support only. This is not legal advice, compliance certification, or proof that SLA workflows are complete.</p>
      <div className="mt-7 grid gap-4 sm:grid-cols-4">
        {rows.length ? rows.map((row) => <section className="card p-5" key={row.status}><p className="text-sm text-slate-200">{row.status}</p><p className="mt-2 text-2xl font-bold">{Number(row.count)}</p></section>) : <p className="text-slate-300">No privacy incidents recorded.</p>}
      </div>
