...
import { isLockdownEnforcing } from "./scanner";
import { DEFAULT_EXTENSION_API_BASE_URL } from "../../../../packages/shared/src/constants";

/**
 * Trial mode is enrollment-shaped but not an enrollment.
 *
 * `startTrialMode()` writes `enrollmentStatus: "enrolled"` so the rest of the UI treats the
 * sample policy as live. The side effect was that `enrollmentMarkup()` — which returns `""`
 * for anyone "enrolled" — permanently deleted the only place in the whole extension where an
 * enrollment code can be entered. A user who clicked "Try Free" once had no path left to
 * connect to their organization short of reinstalling the extension.
 */
export function isTrialMode(state: ExtensionState) {
  return state.config.organizationId === "trial-local";
}

export function enrollmentMarkup(state: ExtensionState) {
  const trial = isTrialMode(state);
  // A real enrollment is the only state with nothing left to offer.
  if (state.enrollmentStatus === "enrolled" && !trial) return "";
  return `
    <div class="card enrollment" data-enrollment-view="${trial ? "trial" : "not-enrolled"}">
      <div class="card-title">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
        Connect to Your Organization
      </div>
      <p class="help" style="margin-top:0;margin-bottom:12px;">${
        trial
          ? "You're on the free trial with a built-in sample policy. Enter an enrollment code to switch to your organization's real policy and audit trail."
          : "Enter the enrollment code supplied by your administrator to activate enterprise protection."
      }</p>
...
export function enrollmentStatusLabel(state: ExtensionState) {
  // Only claim lockdown when a restriction is genuinely in force — see `lockdownRestrictions`.
  if (isLockdownEnforcing(state)) return "Emergency lockdown active";
  if (state.policy?.emergencyLockdown?.enabled) return "Lockdown flagged — no restrictions set";
  if (state.policySyncStatus === "offline") return "Offline cached policy";
  if (state.policySyncStatus === "error") return "Policy sync failed";
  if (isTrialMode(state)) return "Free trial — sample policy";
