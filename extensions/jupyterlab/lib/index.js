import { ICommandPalette, InputDialog, Notification, MainAreaWidget } from '@jupyterlab/apputils';
import { INotebookTracker } from '@jupyterlab/notebook';
import { CodeCell } from '@jupyterlab/cells';
import { ISettingRegistry } from '@jupyterlab/settingregistry';
import { BrokerClient, BrokerError, LocalLoopbackTransport, ServerProxyTransport } from './broker';
...
const plugin = {
    ...
    activate: (app, notebooks, palette, settings) => {
        ...
        const transport = new (class {
            local = new LocalLoopbackTransport(() => brokerPort, async () => pastedToken);
            proxy = new ServerProxyTransport();
            baseUrl() {
        ...
        const announce = (result) => {
            if (result.decision === 'block') {
                ...
                Notification.warning(`SoterAI: ${result.decision} (risk ${result.riskScore}).`, { autoClose: 5000 });
            }
        };
        const handleError = (title, err) => {
            const message = err instanceof BrokerError
                ? err.message
